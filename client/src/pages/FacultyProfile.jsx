...
export default function FacultyProfile() {
  ...
  const API_URL = process.env.REACT_APP_API_URL_FACULTY || 
    (process.env.REACT_APP_API_URL ? `${process.env.REACT_APP_API_URL}/api/faculty` : null) || 
    (process.env.REACT_APP_API_BASE ? `${process.env.REACT_APP_API_BASE}/faculty` : null) || 
    '/api/faculty';
  ...
  return (
    <div className="pt-14 pb-10 min-h-screen bg-gray-900 text-white">
      ...
      <meta name="description" content={`Read reviews and ratings for Prof. ${name} in the ${department || 'VIT-AP'} department. Teaching: ${(teaching_rating || 0).toFixed(2)} ⭐, Correction: ${(correction_rating || 0).toFixed(2)} ⭐, Attendance: ${(attendance_rating || 0).toFixed(2)} ⭐.`} />
      ...
      <div className="max-w-3xl mx-auto bg-gray-800 rounded-lg shadow-xl p-6 mt-6">
        ...
        <div className="mb-6 space-y-2 text-gray-300 text-lg">
          <p>
            <strong>Teaching:</strong> {(teaching_rating || 0).toFixed(2)} ⭐ ({num_teaching_ratings})
          </p>
          <p>
            <strong>Correction:</strong> {(correction_rating || 0).toFixed(2)} ⭐ ({num_correction_ratings})
          </p>
          <p>
            <strong>Attendance:</strong> {(attendance_rating || 0).toFixed(2)} ⭐ ({num_attendance_ratings})
          </p>
        </div>
